...
def assignmentParser(mainGraph, leftGraph, assignment,main_app):
    ...
    for i in range(0, len(vertList), 2):
        if vertList[i + 1] in y or vertList[i] in y.values():
            errorHandler("Każdy wierzchołek musi być przyporządkowany do dokładnie jednego wierzchołka",main_app)
            return [],False
        y[vertList[i + 1]] = vertList[i]

    if len(y) != leftGraph.number_of_nodes():
        errorHandler("Przyporządkowanie niezgodne z liczbą wierzchołków grafu lewej strony produkcji",main_app)
        isOk = False
        return [],False

    for n in y.keys():
        if not leftGraph.has_node(n):
            errorHandler("Błędne przyporządkowanie (wierzchołek nie istnieje w grafie lewej strony produkcji)",main_app)
            isOk = False
            return [],False
        if not mainGraph.has_node(y[n]):
            errorHandler("Błędne przyporządkowanie (wierzchołek nie istnieje w grafie głównym)",main_app)
            isOk = False
            return [],False
        if leftGraph.nodes[n]['label'] != mainGraph.nodes[y[n]]['label']:
            errorHandler("Błędne przyporządkowanie (niezgodne etykiety wierzchołków)",main_app)
            isOk = False
            return [],False

    for edge in leftGraph.edges:
        if not mainGraph.has_edge(y[edge[0]], y[edge[1]]):
            errorHandler("Błędne przyporządkowanie (krawędź nie istnieje w głównym grafie)",main_app)
            isOk = False
            return [],False
...
def errorHandler(errorMessage,main_app):
    main_app.choice.errorNoticed(errorMessage)
    # print(errorMessage)
    return
...
# assignmentParser(mainGraph, leftGraph, assignment,main_app)
